...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    return Response.json({ ok: true, database: "ready", providers: { clerk: Boolean(process.env.CLERK_SECRET_KEY), dodo: hasDodoConfig(), resend: Boolean(process.env.RESEND_API_KEY), evidence: Boolean(process.env.FIRECRAWL_API_KEY || process.env.EXA_API_KEY), firecrawl: Boolean(process.env.FIRECRAWL_API_KEY), exa: Boolean(process.env.EXA_API_KEY), openrouter: Boolean(process.env.OPENROUTER_API_KEY), clarity: Boolean(process.env.NEXT_PUBLIC_CLARITY_PROJECT_ID) } });
  } catch {
