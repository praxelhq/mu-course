...
import { findOpportunity } from "@/lib/opportunities";
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (parsed.data.blueprintSlug && !findBlueprint(parsed.data.blueprintSlug) && !findOpportunity(parsed.data.blueprintSlug)) return Response.json({ error: "Unknown product event." }, { status: 400 });
  try {
