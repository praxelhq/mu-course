...
import { AlertTriangle, ArrowRight, Check, CheckCircle2, ChevronDown, Circle, Copy, Download, ExternalLink, LoaderCircle, Plus, RefreshCw, Save, Sparkles, Trash2 } from "lucide-react";
...
type Project = {
  ...
  promptSets: { id: string; model: string; platform: string; templateVersion: string; content: PromptSetContent }[];
...
export function Workspace(): React.ReactNode {
  ...
  return (
    <main className="workspace" data-clarity-mask="true">
      <header className="workspace-header"><Brand /><div className="project-title">{project?.name ?? "New project"}<ChevronDown size={15} /></div><div className="workspace-status">{project ? <><span className={`status-dot ${project.status}`} />{project.status}</> : "Private workspace"}</div></header>
      ...
      {!loading && project && (["analyzing", "failed"] as string[]).includes(project.status) ? <AnalysisState project={project} onNew={() => setProject(null)} /> : null}
...
function UnderstandingEditor({ project, onReload }: { project: Project; onReload: () => Promise<void> }): React.ReactNode {
  ...
  async function save(): Promise<number> { setBusy("save"); const result = await jsonRequest<{ version: number }>(`/api/projects/${project.id}/understanding`, { method: "PUT", body: JSON.stringify(value) }); setBusy(null); return result.version; }
  async function approve() { setBusy("approve"); setMessage(null); try { if (JSON.stringify(value) !== JSON.stringify(latest.content)) await save(); await jsonRequest(`/api/projects/${project.id}/approve`, { method: "POST" }); track("understanding_approved"); await onReload(); } catch (reason) { setMessage(reason instanceof Error ? reason.message : "Approval failed."); } finally { setBusy(null); } }
...
function PromptSequence({ project, entitled, availableLicenses, lockedPromptCount, onReload }: { project: Project; entitled: boolean; availableLicenses: number; lockedPromptCount: number; onReload: () => Promise<void> }): React.ReactNode {
  ...
  const [selected, setSelected] = useState(0); const [copied, setCopied] = useState<number | "all" | null>(null); const [completed, setCompleted] = useState<Set<number>>(() => new Set());
  ...
  return <section className="prompt-shell"><aside className="prompt-nav"><span>BUILD SEQUENCE</span>{prompts.map((item, index) => <button className={selected === index ? "selected" : ""} onClick={() => setSelected(index)} key={item.order}><b>{String(item.order).padStart(2, "0")}</b><div><strong>{item.title}</strong><small>{completed.has(index) ? "Completed" : index === selected ? "Selected" : item.purpose}</small></div>{completed.has(index) ? <Check size={14} /> : <Circle size={14} />}</button>)}{!entitled ? <div className="locked-nav-item"><span>{lockedPromptCount}</span><strong>follow-up prompts locked</strong></div> : null}</aside><div className="prompt-main"><div className="screen-title"><div><h1>{entitled ? "Your build sequence" : "Your free base prompt"}</h1><p>{entitled ? "Follow these prompts in order. Each builds on the previous one." : "Use this prompt now. Unlock only if the product model and first output earn it."}</p></div><div className="screen-actions"><button className="button primary" onClick={() => copyText(current.prompt, selected)}><Copy size={16} />{copied === selected ? "Copied" : "Copy prompt"}</button>{entitled ? <><button className="button secondary" onClick={() => copyText(allText, "all")}><Copy size={16} />{copied === "all" ? "Copied all" : "Copy all"}</button><a className="button secondary" download={`${project.name.replace(/\W+/g, "-").toLowerCase()}-prompts.txt`} href={`data:text/plain;charset=utf-8,${encodeURIComponent(allText)}`}><Download size={16} /> Export</a></> : null}</div></div><article className="prompt-canvas"><header><span>{String(current.order).padStart(2, "0")}</span><h2>{current.title}</h2><button onClick={() => setCompleted((previous) => { const next = new Set(previous); if (next.has(selected)) next.delete(selected); else next.add(selected); return next; })}>{completed.has(selected) ? <CheckCircle2 size={17} /> : <Circle size={17} />} {completed.has(selected) ? "Completed" : "Mark complete"}</button></header><h3>Outcome</h3><p>{current.purpose}</p><h3>Build instructions</h3><pre>{current.prompt}</pre><h3>Acceptance check</h3><ul>{current.completionChecks.map((check) => <li key={check}><Circle size={13} />{check}</li>)}</ul></article>{entitled && prompts[selected + 1] ? <button className="next-up" onClick={() => setSelected(selected + 1)}><span>Next up</span><b>{String(prompts[selected + 1].order).padStart(2, "0")}</b><strong>{prompts[selected + 1].title}</strong><ArrowRight size={18} /></button> : null}{!entitled ? <UpgradePanel projectId={project.id} availableLicenses={availableLicenses} lockedPromptCount={lockedPromptCount} onUnlocked={onReload} /> : null}</div><aside className="lineage-rail"><span>UNDERSTANDING LINEAGE</span><div className="lineage-step"><Check size={14} /><div><strong>Source brief</strong><small>Captured</small></div></div><div className="lineage-step"><Check size={14} /><div><strong>Understanding v{project.approvedVersion}</strong><small>Approved</small></div></div><dl><dt>Model</dt><dd>{set.model}</dd><dt>Provider</dt><dd>OpenRouter</dd><dt>Platform</dt><dd>{set.platform}</dd><dt>Access</dt><dd>{entitled ? "Project unlocked" : "Base prompt free"}</dd><dt>Template</dt><dd>{set.templateVersion}</dd></dl><button className="share-link" onClick={() => copyText(window.location.href, "all")}><ExternalLink size={14} /> Copy private link</button><button className="delete-link" onClick={async () => { if (window.confirm("Delete this project and all generated artifacts?")) { await fetch(`/api/projects/${project.id}`, { method: "DELETE" }); window.location.assign("/workspace"); } }}><Trash2 size={14} /> Delete project</button></aside></section>;
}
