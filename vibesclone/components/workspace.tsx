...
import { track, trackProductEvent } from "@/components/analytics/clarity";
...
type Project = {
  ...
  publicId?: string | null; isPublic?: boolean; publishedAt?: string | null; publishedVersion?: number | null; remixOrigin?: string | null;
...
export function Workspace(): React.ReactNode {
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      const hasPrefill = Boolean(new URLSearchParams(window.location.search).get("sourceUrl"));
      if (id) await loadProject(id);
      else if (hasPrefill) return;
      else {
...
function ProjectSetup({ onCreated }: { onCreated: (project: Project) => void }): React.ReactNode {
  ...
  const prefill = useMemo(() => { const query = new URLSearchParams(search); return { sourceUrl: query.get("sourceUrl") ?? "", niche: query.get("niche") ?? "", usp: query.get("usp") ?? "", remixOrigin: query.get("origin") ?? "" }; }, [search]);
  const [edits, setEdits] = useState<Partial<typeof prefill>>({});
  ...
  return <section className="setup-panel"><div className="setup-copy"><h1>{prefill.remixOrigin ? "Remix the product logic.\nChoose your advantage." : "Start with the product.\nChange the reason to choose it."}</h1><p>{prefill.remixOrigin ? "We carried the safe starting context into a new private project. Change any field before analysis—nothing from the original builder’s paid sequence is included." : "Give VibesClone one public source, the audience you want, and the USP you’ll compete on. You’ll verify the interpretation before any prompt is generated."}</p>{prefill.remixOrigin ? <span className="remix-origin">STARTED FROM · {prefill.remixOrigin.replace(":", " / ")}</span> : null}</div><form action={submit} className="project-form"><input type="hidden" name="remixOrigin" value={prefill.remixOrigin} /><label>Product URL<input name="sourceUrl" type="url" required placeholder="https://linear.app" value={edits.sourceUrl ?? prefill.sourceUrl} onChange={(event) => setEdits({ ...edits, sourceUrl: event.target.value })} /></label><label>Reference UI URL <span>optional</span><input name="uiReferenceUrl" type="url" placeholder="Public Stitch or inspiration link" /></label><div className="form-split"><label>Target niche<input name="niche" required minLength={2} placeholder="Independent recruiters" value={edits.niche ?? prefill.niche} onChange={(event) => setEdits({ ...edits, niche: event.target.value })} /></label><label>USP direction<input name="usp" required minLength={2} placeholder="Local-first and radically fast" value={edits.usp ?? prefill.usp} onChange={(event) => setEdits({ ...edits, usp: event.target.value })} /></label></div><fieldset><legend>Build target</legend><div className="target-options">{targets.map((target, index) => <label key={target.value}><input type="radio" name="buildTarget" value={target.value} defaultChecked={index === 3} /><span>{target.label}</span></label>)}</div></fieldset>{message ? <p className="form-error">{message}</p> : null}<button className="button primary wide" disabled={submitting}>{submitting ? <LoaderCircle className="spin" size={18} /> : <Sparkles size={18} />}{submitting ? "Starting analysis" : "Analyze product"}</button></form></section>;
}
...
function PromptSequence({ project, entitled, availableLicenses, lockedPromptCount, onReload }: { project: Project; entitled: boolean; availableLicenses: number; lockedPromptCount: number; onReload: () => Promise<void> }): React.ReactNode {
  ...
  const [copied, setCopied] = useState<number | "all" | "link" | null>(null);
  ...
  const copyText = useCallback(async (text: string, marker: number | "all" | "link") => { await copyToClipboard(text); setCopied(marker); if (marker === "link") trackProductEvent("public_report_shared", { publicId: project.publicId ?? undefined }); else trackProductEvent("prompt_copied"); window.setTimeout(() => setCopied(null), 1800); }, [project.publicId]);
