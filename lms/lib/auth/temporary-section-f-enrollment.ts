...
export type TemporaryEnrollmentDeps = {
  findSectionF: () => Promise<{ id: string } | null>;
  createUser: (data: {
    email: string;
    name: string;
    sectionId: string;
...
export function isTemporarySectionFEnrollmentOpen(
  ...
  const raw = env.TEMPORARY_SECTION_F_ENROLLMENT_UNTIL;
  if (!raw) return false;
...
export async function enrollTemporarySectionFUser(
  ...
  const section = await deps.findSectionF();
  if (!section) throw new Error("Temporary Section F enrollment is unavailable");
...
export function prismaTemporaryEnrollmentDeps(
  ...
  return {
    findSectionF: () => db.section.findUnique({ where: { code: "F" }, select: { id: true } }),
    ...
    createAuditLog: async ({ userId, email, expiresAt }) => {
      await db.auditLog.create({
        data: {
          action: "auth.temporary_section_f_enrollment",
          targetType: "user",
          targetId: userId,
          after: { email, sectionCode: "F", source: "auth", expiresAt },
        },
      });
    },
  };
