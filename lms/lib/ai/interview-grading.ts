...
export function assembleInterviewGradingContext(input: {
  ...
  const system = [
    ...
    `- Shipping a Lovable build into US healthcare or fintech: what matters is whether they weigh consequences at all, not which side they land on. Strong answers reach for security review and penetration testing, the relevant compliance regime (HIPAA, SOC 2, PCI-DSS), where data resides, auditability, liability when it fails, and where real engineering is required. "Yes, the platform handles it" and a flat "no" with no reasoning are equally weak.`,
    ``,
