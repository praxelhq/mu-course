  ...
  for (const row of newestFirst) {
    ...
    const fields =
      row.fields && typeof row.fields === "object" && !Array.isArray(row.fields)
        ...
        : {};
    // Match the production protected image route exactly: Session 2 images
    // use fields.image, with the first uploaded file as the legacy fallback.
    const imageKey =
      typeof fields.image === "string" && fields.image ? fields.image : (row.files[0] ?? null);
